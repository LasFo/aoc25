...
pub fn main() !void {
    ...
    for(0..1000) |i| {
        ...
        _=d.merge(di.idx_a, di.idx_b);
    }
    var leaders = std.AutoArrayHashMap(usize, void).init(alloc);
    defer leaders.deinit();
    for(0..poss.items.len) |i| {
        try leaders.put(d.leader(i), {});
    }
    var sizes = std.ArrayList(usize){};
    defer sizes.deinit(alloc);
    var it = leaders.iterator();
    while(it.next()) |f| {
        try sizes.append(alloc, d.size(f.key_ptr.*));
    }
    std.mem.sort(usize, sizes.items, {}, std.sort.desc(usize));
    var res: usize = 1;
    for(0..3) |i| res*=sizes.items[i];
    std.debug.print("{}\n", .{res});
}
